...
function generateFood(){
    ...
    if((snakeGame.snakeHead.position.x == snakeGame.food.position.x)&&(snakeGame.snakeHead.position.y == snakeGame.food.position.y)){
        eatingAudio.play();
        if(snakeDeque.isEmpty()){
            snakeGame.snakeHeadX = snakeGame.snakeHead.position.x;
            snakeGame.snakeHeadY = snakeGame.snakeHead.position.y;
        }else{
            snakeGame.snakeHeadX = snakeDeque.getBack().position.x;
            snakeGame.snakeHeadY = snakeDeque.getBack().position.y;
        }
        snakeDeque.insertBack(createSnakeBody());
        snakeDeque.getBack().position.set(snakeGame.snakeHeadX - snakeGame.speed.x, snakeGame.snakeHeadY - snakeGame.speed.y, .51);
        scene.add( snakeDeque.getBack());
        snakeGame.food.position.set(getRandomIndex(), getRandomIndex(), .51);
    }

    for(var i = 0 ; i < snakeDeque.size(); i++){
        X = snakeDeque.getValues()[i].position.x;
        Y = snakeDeque.getValues()[i].position.y;
        if( (( snakeDeque.getValues()[i].position.x == snakeGame.food.position.x) && ( snakeDeque.getValues()[i].position.y == snakeGame.food.position.y)) ){
            snakeGame.food.position.set(getRandomIndex(), getRandomIndex(), .51);
          scene.add(snakeGame.food);
        }
  }
...
function gameOver(){
    ...
    if((snakeGame.snakeHead.position.x > 4.5)||(snakeGame.snakeHead.position.x< -4.5)||(snakeGame.snakeHead.position.y>4.5)||(snakeGame.snakeHead.position.y<-4.5)){
        ...
        gameOverAudio.play();
        alert("Game Over \n\nYour score: " + snakeDeque.size());
        return;
    }
    for(let i = 2 ; i<snakeDeque.size(); i++){

        snakeGame.snakeBodyX = snakeDeque.getValues()[i].position.x;
        snakeGame.snakeBodyY = snakeDeque.getValues()[i].position.y;
...
const snakeDeque = new Deque();
let radius = 7;
let w = Math.PI;
function render() {
    requestAnimationFrame(render);
    const t = clock.getElapsedTime();
    camera.position.x = radius * Math.cos(w * t);
    camera.position.y = radius * Math.sin(w * t);
...
function onDocumentKeyDown(event) {
    var input = event.key;
    if (!snakeDeque.isEmpty()) {
        if (input === "ArrowDown" && snakeDeque.getFront().position.y != (snakeGame.snakeHead.position.y - 1)) {
            snakeGame.speed.y = -1;
            snakeGame.speed.x = 0;
        }
        if (input === "ArrowUp" && snakeDeque.getFront().position.y != (snakeGame.snakeHead.position.y + 1)) {
            snakeGame.speed.y = 1;
            snakeGame.speed.x = 0;
        }
        if (input === "ArrowLeft" && snakeDeque.getFront().position.x != (snakeGame.snakeHead.position.x - 1)) {
            snakeGame.speed.x = - 1;
            snakeGame.speed.y = 0;
        }
        if(input === "ArrowRight" && snakeDeque.getFront().position.x != (snakeGame.snakeHead.position.x + 1)){
            snakeGame.speed.x = 1;
            snakeGame.speed.y = 0;
        }
    } else {
...
function move(){
    ...
    snakeDeque.insertFront(createSnakeBody());
    snakeDeque.getFront().position.set(snakeGame.snakeHeadX , snakeGame.snakeHeadY ,0.51);
    scene.add( snakeDeque.getFront() );
    scene.remove(snakeDeque.getBack());
    snakeDeque.removeBack();
    generateFood();
    gameOver();
}
